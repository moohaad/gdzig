...
The extension side is three hand-written files and a generated descriptor. The
...
mygame/
  ...
  project/mygame.gdextension  generated by `zig build`
...
Passing `godot_project` to the dependency reads the current Input Map into
`godot.input.Action`, catalogs `.tscn` files for cross-scene typing, and enables
compile-time `res://` checks. For example,
`godot.input.Action.jump` exists only while `jump` is configured in
...
implemented by your extension rather than Godot, use `SceneWith` and provide a
namespace such as `struct { pub const Player = PlayerNode; }`.

**`project/mygame.gdextension`.** Generated by `addExtension`. Its entry symbol comes
from `.entry_symbol`, and its library selector comes from the resolved target and the
actual output filename, so those values cannot drift apart. Install `extension.manifest`
beside `project.godot`, as above. If the library goes somewhere other than `lib/`, set
`.manifest = .{ .library_dir = "path" }` on `addExtension` and install it there.
...
The manifest defaults to `reloadable = true`, which opts into hot reload. Set
`.manifest = .{ .reloadable = false }` to disable it. Exported properties and the instances
