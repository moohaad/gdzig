/// The identity [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d). This is a transform with no translation, no rotation, and a scale of `Vector3.ONE`. Its `basis` is equal to `Basis.IDENTITY`. This also means that:
///
...
/// [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d) with mirroring applied perpendicular to the YZ plane. Its `basis` is equal to `Basis.FLIP_X`.
pub const flip_x: Transform3d = .initBasisOrigin(Basis.flip_x, Vector3.zero);

/// [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d) with mirroring applied perpendicular to the XZ plane. Its `basis` is equal to `Basis.FLIP_Y`.
pub const flip_y: Transform3d = .initBasisOrigin(Basis.flip_y, Vector3.zero);

/// [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d) with mirroring applied perpendicular to the XY plane. Its `basis` is equal to `Basis.FLIP_Z`.
...
/// Constructs a [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d) identical to `IDENTITY`.
///
/// **Note:** In C#, this constructs a [Transform3D](https://gdzig.github.io/gdzig/#gdzig.builtin.transform3d.Transform3d) with its `origin` and the components of its `basis` set to `Vector3.ZERO`.
